...
#cursor_a {position:absolute;z-index:1000;width:400px;cursor:url('<?php echo DT_SKIN;?>image/prev.cur'),default;}
#cursor_b {position:absolute;z-index:1000;width:400px;cursor:url('<?php echo DT_SKIN;?>image/next.cur'),default;margin-left:400px;}
</style>
<?php if($view) { ?>
...
图片：<span id="hits" class="px11"><?php echo $items;?></span>&nbsp;&nbsp;&nbsp;
...
日期：<span class="px11"><?php echo $adddate;?></span>&nbsp;&nbsp;&nbsp;
点击：<span id="hits" class="px11"><?php echo $hits;?></span>&nbsp;&nbsp;&nbsp;
<span onclick="window.open('<?php echo $MODULE['3']['linkurl'];?>view.php?img='+Dd('destoon_photo').src);" class="c_p">查看原图</span>&nbsp;&nbsp;&nbsp;
...
<div class="main_body"><script type="text/javascript">Df('<?php echo $MODULE['3']['linkurl'];?>comment.php?mid=<?php echo $moduleid;?>&itemid=<?php echo $itemid;?>', 'id="destoon_comment" style="width:100%;"');</script></div>
</div>
<?php } ?>
...
<a href="<?php echo $v['linkurl'];?>"><img src="<?php echo $v['thumb'];?>" width="120" height="90" alt="<?php echo $v['alt'];?>" title="<?php echo $v['alt'];?>"/></a>
<div><a href="<?php echo $v['linkurl'];?>" title="<?php echo $v['alt'];?>"><?php echo $v['title'];?></a></div>
<p><?php echo $v['items'];?>图<?php if($v['open']<3) { ?> <img src="<?php echo $MODULE['2']['linkurl'];?>image/ico_lock.gif" style="border:none;padding:0;" align="absmiddle" title="访问受限"/><?php } ?>
</p>
...
<?php if($itemid) { ?><script type="text/javascript">Dd('side').style.display = Dd('split').style.display = 'none';</script><?php } ?>
