...
<script type="text/javascript">s('auction');m('<?php echo $nav;?>');</script>
...
<tr>
<td class="tl">订单单号</td>
<td class="tr">T<?php echo $td['itemid'];?> <?php if($DT['auction']) { ?>(<?php echo $DT['auction_nm'];?>订单单号:<?php echo $td['auction_no'];?>)<?php } ?>
</td>
</tr>
<tr>
<td class="tl">商品名称</td>
...
<input type="button" value=" 重 置 " class="btn" onclick="Go('auction.php?action=<?php echo $action;?>');"/><br/>
<div class="b10"></div>
...
<span class="f_r">
<?php if($v['status'] == 1) { ?>
<input type="button" value="确认收货" class="btn" onclick="if(confirm('您确定已经收到商家货物且质量与数量无误吗？此操作将不可撤销'))Go('auction.php?itemid=<?php echo $v['itemid'];?>&action=update&step=receive&page=<?php echo $page;?>');"/>
<?php } else if($v['status'] == 2) { ?>
<input type="button" value="确认消费" class="btn" onclick="if(confirm('您确定已在商家消费且质量与数量无误吗？此操作将不可撤销'))Go('auction.php?itemid=<?php echo $v['itemid'];?>&action=update&step=used&page=<?php echo $page;?>');"/>
<?php } ?>
<input type="button" value="订单详情" class="btn" onclick="Go('auction.php?itemid=<?php echo $v['itemid'];?>&action=update&step=detail');"/>
&nbsp;
</span>
&nbsp;
<strong>单号：</strong><?php echo $v['itemid'];?>&nbsp;&nbsp;
<!-- <strong>密码：</strong><?php echo $v['password'];?>&nbsp;&nbsp; -->
<!-- <strong>单价：</strong>￥<?php echo $v['auction_price'];?>&nbsp;&nbsp; -->
...
<?php if($v['status'] == 0) { ?>
<?php if($v['logistic']) { ?>
<input type="button" value="确认发货" class="btn" onclick="Go('auction.php?itemid=<?php echo $v['itemid'];?>&action=update&step=send&page=<?php echo $page;?>');"/>
<?php } else { ?>
<input type="button" value="已消费" class="btn" onclick="if(confirm('您确定买家已经消费吗？此操作将不可撤销'))Go('auction.php?itemid=<?php echo $v['itemid'];?>&action=update&step=used&page=<?php echo $page;?>');"/>
<?php } ?>
<?php } ?>
<input type="button" value="订单详情" class="btn" onclick="Go('auction.php?itemid=<?php echo $v['itemid'];?>&action=update&step=detail');"/>
&nbsp;
