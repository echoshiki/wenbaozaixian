...
<span class="f_r">
<?php if($v['status'] == 1) { ?>
<input type="button" value="确认收货" class="btn" onclick="if(confirm('您确定已经收到商家货物且质量与数量无误吗？此操作将不可撤销'))Go('group.php?itemid=<?php echo $v['itemid'];?>&action=update&step=receive&page=<?php echo $page;?>');"/>
<?php } else if($v['status'] == 2) { ?>
<input type="button" value="确认消费" class="btn" onclick="if(confirm('您确定已在商家消费且质量与数量无误吗？此操作将不可撤销'))Go('group.php?itemid=<?php echo $v['itemid'];?>&action=update&step=used&page=<?php echo $page;?>');"/>
<?php } ?>
...
<?php if($v['status'] == 0) { ?>
<?php if($v['logistic']) { ?>
<input type="button" value="确认发货" class="btn" onclick="Go('group.php?itemid=<?php echo $v['itemid'];?>&action=update&step=send&page=<?php echo $page;?>');"/>
<?php } else { ?>
<input type="button" value="已消费" class="btn" onclick="if(confirm('您确定买家已经消费吗？此操作将不可撤销'))Go('group.php?itemid=<?php echo $v['itemid'];?>&action=update&step=used&page=<?php echo $page;?>');"/>
<?php } ?>
<?php } ?>
<input type="button" value="订单详情" class="btn" onclick="Go('group.php?itemid=<?php echo $v['itemid'];?>&action=update&step=detail');"/>
&nbsp;
