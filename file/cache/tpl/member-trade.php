...
<tr>
<td class="tab" id="action"><a href="trade.php"><span>收到的订单(卖家)</span></a></td>
<td class="tab_nav">&nbsp;</td>
<td class="tab" id="action_order"><a href="trade.php?action=order"><span>发出的订单(买家)</span></a></td>
<?php if($DT['trade']) { ?>
<td class="tab_nav">&nbsp;</td>
<td class="tab" id="action_bind"><a href="trade.php?action=bind"><span>绑定<?php echo $DT['trade_nm'];?>帐号</span></a></td>
<?php } ?>
</tr>
</table>
</div>
...
<?php if($td['buyer_receive']) { ?>
<tr>
<td class="tl">买家期望物流</td>
<td class="tr f_red f_b"><?php echo $td['buyer_receive'];?></td>
</tr>
<?php } ?>
<tr>
<td class="tl">物流类型</td>
...
<span class="f_r">
<?php if($v['status'] == 0) { ?>
<input type="button" value="删 除" class="btn" onclick="if(confirm('确实要删除此交易吗？此操作将不可撤销')) Go('trade.php?itemid=<?php echo $v['itemid'];?>&action=update&step=close&forward=<?php echo $forward;?>');"/>
<?php } else if($v['status'] == 1) { ?>
<?php if($DT['trade']) { ?>
<a href="trade.php?itemid=<?php echo $v['itemid'];?>&action=update&step=pay&forward=<?php echo $forward;?>" class="t" onclick="return confirm('确认订单无误，现在付款吗？');" target="_blank"><img src="<?php echo DT_PATH;?>api/trade/<?php echo $DT['trade'];?>/pay.gif" align="absmiddle" title="<?php echo $DT['trade_nm'];?>付款"/></a>
<?php } else { ?>
<input type="button" value="付 款" class="btn" onclick="Go('trade.php?itemid=<?php echo $v['itemid'];?>&action=update&step=pay&forward=<?php echo $forward;?>');"/>
<?php } ?>
<input type="button" value="关闭交易" class="btn" onclick="if(confirm('确实要关闭此交易吗？此操作将不可撤销')) Go('trade.php?itemid=<?php echo $v['itemid'];?>&action=update&step=close&forward=<?php echo $forward;?>');"/>
...
<input type="button" value="确认到货" class="btn" onclick="if(confirm('确认已收到货且质量与数量无误吗？\n\n请注意:确认后您的钱将直接支付给卖家')) Go('trade.php?itemid=<?php echo $v['itemid'];?>&action=update&step=receive_goods&forward=<?php echo $forward;?>');"/>
<input type="button" value="申请退款" class="btn" onclick="Go('trade.php?itemid=<?php echo $v['itemid'];?>&action=update&step=refund&forward=<?php echo $forward;?>');"/>
<?php } else { ?>
...
<input type="button" value="删 除" class="btn" onclick="Go('trade.php?itemid=<?php echo $v['itemid'];?>&action=update&step=close&forward=<?php echo $forward;?>');"/>
<?php } ?>
...
<span class="f_r">
<?php if($v['status'] == 0) { ?>
<input type="button" value="确认订单" class="btn" onclick="if(confirm('确实要确认此订单吗？此操作将不可撤销')) Go('trade.php?itemid=<?php echo $v['itemid'];?>&action=update&step=confirm_order&forward=<?php echo $forward;?>');"/>
<input type="button" value="修改价格" class="btn" onclick="Go('trade.php?itemid=<?php echo $v['itemid'];?>&action=update&step=edit_price');"/>
<input type="button" value="关闭交易" class="btn" onclick="if(confirm('确实要关闭此交易吗？此操作将不可撤销')) Go('trade.php?itemid=<?php echo $v['itemid'];?>&action=update&step=close&forward=<?php echo $forward;?>');"/>
<?php } else if($v['status'] == 1) { ?>
<input type="button" value="修改价格" class="btn" onclick="Go('trade.php?itemid=<?php echo $v['itemid'];?>&action=update&step=edit_price');"/>
<input type="button" value="关闭交易" class="btn" onclick="if(confirm('确实要关闭此交易吗？此操作将不可撤销')) Go('trade.php?itemid=<?php echo $v['itemid'];?>&action=update&step=close&forward=<?php echo $forward;?>');"/>
<?php } else if($v['status'] == 2) { ?>
<input type="button" value="确认发货" class="btn" onclick="Go('trade.php?itemid=<?php echo $v['itemid'];?>&action=update&step=send_goods');"/>
<?php if(!$DT['trade']) { ?><input type="button" value="关闭交易" class="btn" onclick="if(confirm('确实要关闭此交易吗？此操作将不可撤销')) Go('trade.php?itemid=<?php echo $v['itemid'];?>&action=update&step=close&forward=<?php echo $forward;?>');"/><?php } ?>
<?php } else if($v['status'] == 3) { ?>
<?php if($v['lefttime']) { ?>
<span class="f_blue"><img src="<?php echo DT_STATIC;?>file/image/clock.gif" width="12" height="12"/> 距买家处理订单还剩<?php echo $v['lefttime'];?></span>&nbsp;
<input type="button" value="延长时间" class="btn" onclick="Go('trade.php?itemid=<?php echo $v['itemid'];?>&action=update&step=add_time&forward=<?php echo $forward;?>');"/>
<?php } else { ?>
<span class="f_blue">买家处理订单超时</span>
<input type="button" value="直接收款" class="btn" onclick="Go('trade.php?itemid=<?php echo $v['itemid'];?>&action=update&step=get_pay&forward=<?php echo $forward;?>');"/>
<?php } ?>
<?php } else if($v['status'] == 4) { ?>
<?php if($v['buyer_star']) { ?>
<input type="button" value="评价详情" class="btn" onclick="Go('trade.php?itemid=<?php echo $v['itemid'];?>&action=update&step=comment_detail&forward=<?php echo $forward;?>');"/>
