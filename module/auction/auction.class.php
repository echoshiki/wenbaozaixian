...
class auction {
    ...
    function auction($moduleid) {
		...
		$this->db = &$db;
		$this->fields = array('catid','mycatid','areaid','level','title','style','fee','introduce','marketprice','price','savemoney','discount','minamount','amount','logistic','thumb','tag','status','hits','orders','username','fromtime','totime','editor','addtime','edittime','ip','template','linkurl','filepath','note','company','truename','telephone','mobile','address','email','msn','qq','ali','skype');
    }
	...
	function pass($post) {
		...
		if(dround($post['price']) < 0.1) return $this->_(lang('message->pass_auction_price'));
