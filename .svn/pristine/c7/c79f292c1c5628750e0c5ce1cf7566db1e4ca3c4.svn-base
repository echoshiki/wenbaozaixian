...
$MTYPE = get_type('auction-'.$_userid);
require DT_ROOT.'/include/post.func.php';
include load($module.'.lang');
include load('my.lang');
require MD_ROOT.'/auction.class.php';
$do = new auction($moduleid);
...
if(check_group($_auctionid, $MOD['auction_refresh'])) $MOD['credit_refresh'] = 0;
...
switch($action) {
	case 'add':
		...
		$could_color = check_group($_auctionid, $MOD['auction_color']) && $MOD['credit_color'] && $_userid;
		...
		if($submit) {
			...
			if($do->pass($post)) {
				...
				if(!$CAT || !check_group($_auctionid, $CAT['auction_add'])) dalert(lang($L['auction_add'], array($CAT['catname'])));
				if($MOD['upload_thumb'] && $_userid && strlen($post['thumb']) < 5) dalert($L['auction_upload_image']);
			...
			$item = array();
			$mycatid_select = type_select('auction-'.$_userid, 0, 'post[mycatid]', $L['type_default']);
		}
	break;
	case 'edit':
		...
		if($submit) {
			if($do->pass($post)) {
				...
				$post['hits'] = $item['hits'];
				//$post['mycatid'] = $_POST['post']['mycatid'];
			...
			$mycatid_select = type_select('auction-'.$_userid, 0, 'post[mycatid]', $L['type_default'], $mycatid);
		}
	break;
	case 'delete':
	...
	default:
		...
		$_process = $L['auction_process'];
		$sorder  = $L['auction_orderby'];
		...
		in_array($status, array(1, 2, 3, 4, 5)) or $status = 3;
		...
		if ($status == 5) {
			$lists = $do->get_auction_list($_username);
		}else{
...
if($_userid) {
	...
	$nums[9] = $r['num'];
	$nums[0] = count($MTYPE);
	$rs = $db->get_one("SELECT COUNT(*) AS num FROM {$table}_list WHERE auction_user='$_username'");
	$nums[5] = $rs['num']; 
}
if ($status == 5) {
	include template('my_'.$module.'_list', 'member');
}else{
